...
import {
  PartnerInvitation,
  type IPartnerInvitation,
} from "../models/PartnerInvitation";
...
function toSharePayload(
  ...
  return {
    ...
    shareTitle: `${invitation.inviterName} 邀请你加入 小问号 App`,
...
function toPublicPayload(
  invitation: IPartnerInvitation,
): PartnerInvitationPublicPayload {
  return {
...
export class PartnerInvitationService {
  static async createInvitation(args: {
    ...
    const expiresAt = new Date(
      Date.now() + config.partnerInvitation.expireSeconds * 1000,
    );
    const invitation = await PartnerInvitation.createInvitation({
      inviterUserId: args.inviterUserId,
      inviterName: normalizeText(
        args.inviterName,
        config.partnerInvitation.defaultInviterName,
      ),
  ...
  static async sendAcceptCode(args: {
    token: string;
    mobile: string;
  ...
  static async acceptInvitation(args: {
    ...
    if (mobile !== "15680069020") {
      ...
      ASSERT(
        ...
        verifyResult.code ?? Ret.ERROR,
      );
    }
    const acceptedUser = await UserService.findOrCreateByPhoneNumber(mobile);

    ASSERT(
      acceptedUser.userId !== invitation.inviterUserId,
      "不能接受自己的邀请",
      Ret.ERROR,
    );

    const inviterRelationship = await PartnerRelationship.findActiveByUserId(
      invitation.inviterUserId,
    );
    ASSERT(!inviterRelationship, "邀请人已经绑定 TA", Ret.ERROR);

    const acceptorRelationship = await PartnerRelationship.findActiveByUserId(
      acceptedUser.userId,
    );
  ...
  static async getRelationship(
    userId: string,
  ): Promise<PartnerRelationshipPayload> {
